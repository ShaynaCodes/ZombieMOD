...
void soldier_fire (edict_t *self, int flash_number)
{
	...
	if (self->s.skinnum <= 1)
	{
		monster_fire_blaster (self, start, aim, 500, 600, flash_index, EF_BLASTER);
	}
	else if (self->s.skinnum <= 3)
	{
		monster_fire_shotgun (self, start, aim, 500, 100, DEFAULT_SHOTGUN_HSPREAD, DEFAULT_SHOTGUN_VSPREAD, DEFAULT_SHOTGUN_COUNT, flash_index);
	}
	else
	{
		...
		monster_fire_bullet (self, start, aim, 500, 4, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, flash_index);
